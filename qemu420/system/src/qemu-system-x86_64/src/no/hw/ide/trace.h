...
#define TRACE_IDE_CMD_WRITE_BACKEND_DSTATE() ( \
    trace_event_get_state_dynamic_by_id(TRACE_IDE_CMD_WRITE) || \
    false)

static inline void _nocheck__trace_ide_cmd_write(uint32_t addr, uint32_t val, void * bus)
{
    if (trace_event_get_state(TRACE_IDE_CMD_WRITE) && qemu_loglevel_mask(LOG_TRACE)) {
        struct timeval _now;
        gettimeofday(&_now, NULL);
        qemu_log("%d@%zu.%06zu:ide_cmd_write " "IDE PIO wr @ 0x%"PRIx32" (Device Control); val 0x%02"PRIx32"; bus %p" "\n",
                 qemu_get_thread_id(),
                 (size_t)_now.tv_sec, (size_t)_now.tv_usec
                 , addr, val, bus);
    }
