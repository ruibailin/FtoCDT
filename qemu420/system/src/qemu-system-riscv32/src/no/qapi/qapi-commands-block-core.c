...
void qmp_marshal_block_latency_histogram_set(QDict *args, QObject **ret, Error **errp)
{
    ...
    visit_type_q_obj_block_latency_histogram_set_arg_members(v, &arg, &err);
    if (!err) {
...
static void qmp_marshal_output_XDbgBlockGraph(XDbgBlockGraph *ret_in, QObject **ret_out, Error **errp)
{
    Error *err = NULL;
    Visitor *v;

    v = qobject_output_visitor_new(ret_out);
    visit_type_XDbgBlockGraph(v, "unused", &ret_in, &err);
    if (!err) {
        visit_complete(v, ret_out);
    }
    error_propagate(errp, err);
...
void qmp_marshal_x_debug_query_block_graph(QDict *args, QObject **ret, Error **errp)
{
    ...
    qmp_marshal_output_XDbgBlockGraph(retval, ret, &err);
...
void qmp_marshal_block_dirty_bitmap_merge(QDict *args, QObject **ret, Error **errp)
{
    ...
    visit_type_BlockDirtyBitmapMerge_members(v, &arg, &err);
    if (!err) {
        visit_check_struct(v, &err);
    }
    visit_end_struct(v, NULL);
...
void qmp_marshal_blockdev_create(QDict *args, QObject **ret, Error **errp)
{
    ...
    visit_type_q_obj_blockdev_create_arg_members(v, &arg, &err);
    if (!err) {
        visit_check_struct(v, &err);
    }
    visit_end_struct(v, NULL);
