...
void visit_type_NetdevUserOptions_members(Visitor *v, NetdevUserOptions *obj, Error **errp)
{
    ...
    if (visit_optional(v, "domainname", &obj->has_domainname)) {
        visit_type_str(v, "domainname", &obj->domainname, &err);
        if (err) {
            goto out;
        }
    }
    ...
    if (visit_optional(v, "tftp-server-name", &obj->has_tftp_server_name)) {
        visit_type_str(v, "tftp-server-name", &obj->tftp_server_name, &err);
        if (err) {
            goto out;
        }
    }
...
void visit_type_AnnounceParameters_members(Visitor *v, AnnounceParameters *obj, Error **errp)
{
    Error *err = NULL;

    visit_type_int(v, "initial", &obj->initial, &err);
    if (err) {
        goto out;
    }
    visit_type_int(v, "max", &obj->max, &err);
    if (err) {
        goto out;
    }
    visit_type_int(v, "rounds", &obj->rounds, &err);
    if (err) {
        goto out;
    }
    visit_type_int(v, "step", &obj->step, &err);
    if (err) {
        goto out;
    }
    if (visit_optional(v, "interfaces", &obj->has_interfaces)) {
        visit_type_strList(v, "interfaces", &obj->interfaces, &err);
        if (err) {
            goto out;
        }
    }
...
void visit_type_AnnounceParameters(Visitor *v, const char *name, AnnounceParameters **obj, Error **errp)
{
    Error *err = NULL;

    visit_start_struct(v, name, (void **)obj, sizeof(AnnounceParameters), &err);
    if (err) {
        goto out;
    }
    if (!*obj) {
    ...
    visit_type_AnnounceParameters_members(v, *obj, &err);
    if (err) {
        goto out_obj;
    }
    visit_check_struct(v, &err);
...
void visit_type_q_obj_FAILOVER_NEGOTIATED_arg_members(Visitor *v, q_obj_FAILOVER_NEGOTIATED_arg *obj, Error **errp)
{
    Error *err = NULL;

    visit_type_str(v, "device-id", &obj->device_id, &err);
    if (err) {
        goto out;
    }
