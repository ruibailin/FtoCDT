...
F_UINT pack_sf(uint32_t a_sign, uint32_t a_exp, F_UINT a_mant)
{
...
F_UINT unpack_sf(uint32_t *pa_sign, int32_t *pa_exp,
                               F_UINT a)
{
    *pa_sign = a >> (F_SIZE - 1);
...
F_UINT rshift_rnd(F_UINT a, int d)
{
    F_UINT mask;
    if (d != 0) {
        if (d >= F_SIZE) {
...
F_UINT round_pack_sf(uint32_t a_sign, int a_exp, F_UINT a_mant,
...
F_UINT normalize_sf(uint32_t a_sign, int a_exp, F_UINT a_mant,
                           RoundingModeEnum rm, uint32_t *pfflags)
{
    int shift;
...
F_UINT normalize2_sf(uint32_t a_sign, int a_exp, F_UINT a_mant1, F_UINT a_mant0,
                            RoundingModeEnum rm, uint32_t *pfflags)
{
    int l, shift;
    if (a_mant1 == 0) {
...
F_UINT normalize_subnormal_sf(int32_t *pa_exp, F_UINT a_mant)
{
    int shift;
...
F_UINT mul_u(F_UINT *plow, F_UINT a, F_UINT b)
{
    F_ULONG r;
