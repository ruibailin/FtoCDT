...
class ibex_simple_system : public Vibex_simple_system, public VerilatedToplevel {
 public:
  ibex_simple_system(const char *name = "TOP")
      : Vibex_simple_system(name), VerilatedToplevel() {}
  const char *name() const { return STR_AND_EXPAND(ibex_simple_system); }
  void eval() { Vibex_simple_system::eval(); }
  void final() { Vibex_simple_system::final(); }
  void trace(VerilatedTracer &tfp, int levels, int options = 0) {
#if VM_TRACE == 1
    Vibex_simple_system::trace(static_cast<VM_TRACE_CLASS_NAME *>(tfp),
                                   levels, options);
#else
