...
#include "chipyard.TestHarness.LargeBoomConfig/VTestHarness.h"
int largeboom_main(int argc, char** argv)
{
...
//         !tile->io_success && trace_count < max_cycles)
  while (1)
  {
	  ...
	  if(jtag->done()) break;
	  if(tsi->done()) break;
	  if(tile->io_success) break;
	  if(trace_count >= max_cycles) break;
    tile->clock = 0;
    tile->eval();
#if VM_TRACE
    dump = tfp && trace_count >= start;
